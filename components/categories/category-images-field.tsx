...
import type { CategoryImage } from "@/components/categories/types";
...
export function CategoryImagesField({
  ...
  function handleFilesSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (
      mergedFiles.some(
        (file) => !CATEGORY_IMAGE_TYPES.some((type) => type === file.type),
      )
    ) {
    ...
    if (mergedFiles.some((file) => file.size > MAX_CATEGORY_IMAGE_SIZE)) {
  ...
  return (
    <div className="grid gap-3 lg:col-span-2">
      ...
      <input
        ...
        aria-describedby={error ? "category-images-error" : undefined}
      />
